...
# USER INPUT SHOW    
        ...
        comp_df = pd.DataFrame({"Model": all_model_name,"Probability": model_prob, 'Model-Prediction':class_ans})
        chart = (
        alt.Chart(comp_df).mark_bar().encode(
        x=alt.X("Model:N", title="LAUDEN BHOJYUM"),
        y=alt.Y("Probability:Q", title="NYAN RAJYAMM"),
        tooltip=[alt.Tooltip("Model:N", title="Model"),alt.Tooltip("Probability:Q", title="Probability"),alt.Tooltip("Model-Prediction:N", title="Prediction")]))
